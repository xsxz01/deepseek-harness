...
`host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). A launcher may provide process-local cookie authentication before rows mount; that policy requires `127.0.0.1` with port `0` and rejects unauthorized HTTP requests and upgrades before dispatch. A composition without the policy has no authentication, TLS, or built-in origin policy. `compression` defaults to `none`; the shipped Web bundle selects gzip level 1 with a 1024-byte threshold. The dist location is an assembly fact of the frontend plugin that claims the seat.
