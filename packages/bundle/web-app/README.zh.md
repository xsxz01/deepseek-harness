...
运行 `dsh --profile web`，界面即可供你在浏览器中与 agent（智能体）交互式聊天。默认浏览器仅在你传入 `--open` 时打开界面；本机启动默认不打开浏览器。你会获得会话视图、模型与设置管理以及会话历史，背后与其他表层相同的模型访问、工具与安全默认值。该命令会打印带 token 的启动 URL；浏览器用该 token 换取签名会话 cookie，再重定向到干净的根 URL。你可以从命令行更改端口、关闭浏览器交接并允许额外主机；有意不支持绑定所有网络接口。需要浏览器中的交互式工作时选择它；`dsh-headless` 是一次性的命令行兄弟表层。
...
启动后你会看到 `dsh web:` 行，其根 URL 携带新的进程 token。传入 `--open`（且没有 SSH 会话抑制交接）时，默认浏览器会打开该 URL、取得签名 cookie，再重定向到干净的根页面。页面加载且你可以与 agent（智能体）对话，就说明成功了。两种可预期的失败：前端未构建时，启动会以构建提示停止（checkout 中运行 `pnpm run build`）；浏览器无法打开时，stderr 会打印不含凭据的诊断，但服务器会继续运行——请自行打开已打印的启动 URL。

### 配置

大多数用户不需要设置这些；命令行 flag 会提供给下面四个设置——`--host`、`--port` 与 `--trusted-host` 来自本次调用，`--open` 仅为本次调用开启浏览器交接：

| 字段 | 默认值 | 含义 |
|---|---|---|
| `openBrowser` | `false` | 仅显式 `--open` 的本机启动才打开默认浏览器；SSH 启动永不交接 |
...
| [`src/startup.ts`](src/startup.ts) | `web-startup` 提供方：`--host`、`--port`、`--trusted-host`、`--open`、`--help` |
