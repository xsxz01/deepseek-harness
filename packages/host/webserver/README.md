...
`host` accepts exactly two values: `127.0.0.1` (default posture, loopback only) and `0.0.0.0` (deliberate network exposure — the server carries no TLS or origin policy of its own). `port` 0 requests an OS-assigned port; `ctx.webServer.port` reads the listening port afterwards. A launcher may supply process-local cookie authentication through `provideWebServerAuthentication` before the composition loads; that policy requires `127.0.0.1` with port `0` and rejects unauthorized HTTP requests and upgrades before dispatch.
...
`provideWebServerAuthentication(ctx, { cookieName, token })` installs a per-process cookie policy before any route or upgrade dispatches: every HTTP request must carry the exact cookie name and a token compared without data-dependent timing after a length check, and upgrade handshakes without it are closed with a 401. The policy requires a `127.0.0.1` bind with port `0` — a composition violating that constraint fails at load. Providing a second policy throws, and a composition without the policy has no authentication.
...
- **No server-wide TLS or origin policy** — route owners such as `dsh-client-connection` enforce their own request policy. Process-local authentication exists only when a launcher provides it; binding a non-loopback address still exposes unprotected routes and static assets to that network.
