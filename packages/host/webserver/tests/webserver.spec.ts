...
import HttpServer, { provideWebServerAuthentication, renderIndexInjections, type WebServerAuthentication } from '../src/index.ts'
...
async function loadComposition(
  port = 0,
  options: { gzip?: boolean; authentication?: WebServerAuthentication; host?: '127.0.0.1' | '0.0.0.0' } = {},
): Promise<Context> {
  ...
  if (options.authentication !== undefined) {
    provideWebServerAuthentication(context, options.authentication)
  }
...
describe('real Loader composition', () => {
  ...
  it('rejects an authenticated composition that does not request dynamic loopback binding', { timeout: 60_000 }, async () => {
    ...
    await context?.fiber.dispose()
    context = undefined
    await expect(loadComposition(0, { authentication, host: '0.0.0.0' })).rejects.toThrow(
      'authenticated launch requires host 127.0.0.1 and port 0',
    )
  })
