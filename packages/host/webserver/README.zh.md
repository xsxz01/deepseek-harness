...
`host` 只接受两个值：`127.0.0.1`（默认姿态，仅回环）与 `0.0.0.0`（有意向网络开放——服务器自身不携带 TLS 或来源策略）。Launcher 可在组合加载前通过 `provideWebServerAuthentication` 提供进程内 cookie 认证；该策略要求 `127.0.0.1` 与端口 `0`，并在分发前拒绝未认证的 HTTP 请求和 upgrade。`port` 为 0 时请求 OS 分配端口；之后用 `ctx.webServer.port` 读取正在监听的端口。
...
`provideWebServerAuthentication(ctx, { cookieName, token })` 会在任何 route 或 upgrade 分发前安装进程内 cookie 策略：每个 HTTP 请求都必须携带精确的 cookie 名，且 token 在长度检查后以无数据依赖计时的方式比较；未携带的 upgrade 握手会被 401 关闭。该策略要求绑定 `127.0.0.1` 且端口为 `0`——违反该约束的组合在加载时失败。重复提供策略会抛错；未提供策略的组合不具备认证。
...
- **不提供服务器级 TLS 或来源策略**：`dsh-client-connection` 等 route owner 会实施自己的请求策略。进程内认证仅存在于 launcher 提供它时；绑定非回环地址仍会向该网络公开未受保护的 route 与静态资源。
