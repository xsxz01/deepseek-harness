...
- **Keep the web plugins opt-in.** Earlier this note shipped them installed-but-disabled to keep the shipped defaults stable; the product request treats dsh-web-ui and dsh-agent-teams as part of the desktop experience, so the desktop web profile now enables them by default (source checkouts still stay on the in-box stack).
