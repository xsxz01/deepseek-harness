...
stage-runtime specs pin the builtin set and the packed-wins merge; tui specs pin resume-target reading, both env names, workspace-target interception, passthrough, and one-time profile seeding; openpets specs pin executable resolution and staging skip/copy; args specs pin `tui` routing and parent-option rejection. The full host typecheck and the desktop unit suites pass; the packaged artifact re-runs the staged runtime verification and the packaged acceptance suite.
