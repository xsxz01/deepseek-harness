...
A ready event carries the origin, the origin's authenticated URL (the process launch token as its only query input), the Host PID, and the Harness version. The Host mints the token in private process memory; it travels to Electron only through private IPC and leaves the window's address bar in the redirect, in which the Web server exchanges it for the HttpOnly browser-session cookie before index delivery, API routing, static and plugin assets, HMR resources, and WebSocket upgrades. Existing Host and Origin checks continue downstream.
