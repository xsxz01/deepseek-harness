...
> Division of labor: this document = the layering model + the channel-independent RPC protocol; the protocol's Web implementation combines HTTP uplink with the [WebSocket downlink carrier](2026-08-04-websocket-downlink-carrier.md), while the browser object layer is in the [web client architecture note](2026-07-19-gui-web-client-architecture.md). The [Electron desktop distribution](2026-08-14-electron-desktop-distribution.md) reuses that authenticated Web carrier; this note remains authoritative for the channel-independent protocol.
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` supervises the Web Host in a standard Node.js process and loads its authenticated loopback HTTP/WebSocket carrier in Electron.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser and Electron desktop access; zero workspace dependencies (the registry arrives by structural injection); desktop adds private loopback authentication without changing the carrier |
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (see the "Subclass table" below). Electron desktop uses the authenticated browser HTTP/WebSocket carrier.
...
| IPC bridge subclass (hypothetical example — no such shell exists) | a future non-Web shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
