...
## Third-party plugins and the companion app

The packaged runtime does not bundle third-party plugins. Out-of-tree plugins — the dsh-TUI terminal interface (`@deepseek-harness-tui/dsh-tui`), the dsh-web-ui plugin pack (`@linxin666/dsh-web-ui-all`), dsh-agent-teams (`@nanmicoder/dsh-agent-teams`), and dsh-at-file (`dsh-at-file`) — install like any other third-party plugin through the Host-owned plugin marketplace and resolve from any profile through the shared module fallback under `$DSH_HOME/profiles/node_modules`. `dsh tui` (also `dsh.cmd tui`) boots the dsh-TUI profile in the terminal once the published `@deepseek-harness-tui/dsh-tui` package is installed; the launcher fails loud while it is absent. Web plugins are opt-in rows in a profile's `cordis.patch.yml`.
